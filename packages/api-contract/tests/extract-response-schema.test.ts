...
describe("extractResponseSchema", () => {
	it("extracts schema for valid response", () => {
		const schema = extractResponseSchema(CONTRACT_KEYS.GET_SELF, StatusCodes.OK);

		expect(schema).toBeDefined();
	});

	it("returns error message on error codes", () => {
		const schema = extractResponseSchema(CONTRACT_KEYS.GET_SELF, StatusCodes.NOT_FOUND);
