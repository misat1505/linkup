...
export class ChatUtils {
	...
	public getDisplayNameById(id: User["id"]): string | null {
		...
		const userDispayName = user.alias ? user.alias : createFullName(user as User);
	...
	public getImageURL(): User["photoURL"] | null {
		if (this.chat.type === "PRIVATE") {
			if (this.chat.users?.length === 1 && this.chat.users![0].id === this.me.id) {
	...
	public getChatName(): string {
		...
		if (this.chat.type === "GROUP") {
			...
			const names = this.chat.users?.map((user) => `${user.firstName} ${user.lastName}`);
			return names?.join(", ") || "";
		}
		return "";
	}
	...
	public getLastActive(): User["lastActive"] {
		...
		if (this.chat.type === "GROUP") {
			...
			for (const user of otherUsers) {
				if (user.lastActive.getTime() > lastActive.getTime()) lastActive = user.lastActive;
			}
			return lastActive;
		}
		return new Date();
	...
	public getNavigationLastMessageDisplayName(youTranslation?: string): string | null {
		const lastMessage = this.chat.lastMessage;
		if (!lastMessage) return null;

		if (lastMessage.author.id === this.me.id) return youTranslation ?? "You";
		const user = this.chat.users?.find((user) => user.id === lastMessage.author.id) || null;
