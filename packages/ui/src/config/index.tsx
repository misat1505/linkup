...
const UiPackageContext = createContext<UiPackageContextValue | undefined>(undefined);

export const useUiPackageContext = () => {
	const context = useContext(UiPackageContext);
	if (!context) throw new Error("useUiPackageContext called outside UiPackageProvider.");
	return context;
};

export const TRANSLATION_COMPONENT = ({ translationKey, values }: TranslationProps) => {
...
const UiPackageProvider = ({
	...
	return (
		<UiPackageContext.Provider value={{ t: translationFunction, LinkComponent: linkComponent }}>
			{children}
