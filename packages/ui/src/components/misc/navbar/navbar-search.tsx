...
export type NavbarSearchProps = {
	...
	addFriendAction: (meId: User["id"], otherId: User["id"]) => Promise<Friendship | null>;
...
export function NavbarSearch({
	...
	return (
		<Command className="w-60 rounded-lg border bg-white shadow-md dark:bg-black">
			<Tooltip content={<TRANSLATION_COMPONENT translationKey="common.navbar.search.tooltip" />}>
				<CommandInput
			...
			<CommandList
				ref={commandListRef}
				className={cn("no-scrollbar absolute top-14 w-[238px] bg-white shadow-md", {
					hidden: !isExpanded,
				})}
			>
				{isFetching ? (
					...
					<>
						...
						<CommandGroup
							...
							heading={<TRANSLATION_COMPONENT translationKey="common.navbar.search.heading" />}
...
function SearchResultItem({
	...
	const handleAddFriend = async (userId: User["id"]) => {
		...
		if (!friendship)
			return toast({
				variant: "destructive",
				title: t("common.navbar.search.friendships.toasts.already-exists.title"),
				description: t("common.navbar.search.friendships.toasts.already-exists.description", {
		...
		toast({
			title: t("common.navbar.search.friendships.toasts.successfully-created.title"),
			description: t("common.navbar.search.friendships.toasts.successfully-created.description", {
				fullName: createFullName(user),
			}),
			action: (
				<Button onClick={() => navigate("/friends")}>
	...
	return (
		<CommandItem key={user.id} className="flex w-full items-center justify-between">
