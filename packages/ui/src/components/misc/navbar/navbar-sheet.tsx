...
export function NavbarSheet({ user, handleLogout }: NavbarSheetProps) {
	...
	return (
		<Sheet>
			...
			{isLoggedIn ? <LoggedInSheet user={user} handleLogout={handleLogout} /> : <GuestSheet />}
		</Sheet>
	);
}
...
function LoggedInSheet({ user, handleLogout }: { user: User; handleLogout: LogoutFn }) {
	const { t } = useUiPackageContext();
			...
			<SheetDescription className="flex h-full flex-col justify-between pb-8 pt-4 text-sm text-muted-foreground">
				<span>
					{buttons.map((button, idx) => (
						<SheetItem key={idx} text={button.text} Icon={button.icon} href={button.href} />
					))}
				</span>
