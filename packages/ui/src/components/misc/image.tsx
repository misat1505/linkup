...
import { IMAGE_COMPONENT, IS_NEXT_IMAGE, useFetchProtectedURL } from "../../config";
...
function DefaultError({
	...
	return (
		<p className={cn("flex h-full w-full items-center justify-center", className)}>{content}</p>
	);
}
...
export function Image({
	...
	if (!src || isError) {
		return unloader || <DefaultError className={cn(common, errorClasses)} content={errorContent} />;
	}
	...
	return (
		<IMAGE_COMPONENT className={cn("h-full w-full", common, img)} src={data!} alt={src as string} />
	);
}
