...
export function Message({
	...
	return (
		<>
			...
			{isMine ? <MyMessage {...sharedProps} /> : <ForeignMessage {...sharedProps} />}
		</>
	);
}
	...
	return (
		<div className="group flex items-center justify-end gap-x-4">
			...
			<Tooltip content={tooltipText}>
				<div className="flex w-fit max-w-[75%] flex-col items-end" ref={messageRef}>
					{message.response && (
						<>
					...
					{message.content && (
						<div
							className={cn("w-fit rounded-b-md bg-blue-500 px-2 py-1 text-white", {
...
function ForeignMessage({
	...
	const authorDisplayName =
		utils.getDisplayNameById(message.author.id) || createFullName(message.author);
	...
	return (
		...
		>
			<Tooltip content={tooltipText}>
				<div className="flex w-fit max-w-[75%] flex-col items-start" ref={messageRef}>
					{message.response && (
						<div className="ml-10">
							<ResponseText message={message} me={me} chat={chat} />
							<Response
					...
					<div className="flex w-fit items-end gap-x-2">
						...
						<div className="w-fit">
							...
							{message.content && (
								<div
									className={cn("w-fit rounded-b-md bg-slate-200 px-2 py-1 dark:bg-slate-800", {
										"rounded-md": message.files.length === 0,
										"mb-1": message.reactions.length === 0,
									})}
								>
