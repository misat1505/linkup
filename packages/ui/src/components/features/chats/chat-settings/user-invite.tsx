...
type UserAddDialogProps = {
	user: User;
	chatId: Chat["id"];
	addUserToChatAction: (chatId: Chat["id"], userId: User["id"]) => Promise<UserInChat>;
	addUserToChatCb?: (user: UserInChat) => void;
};

function UserAddDialog({ user, chatId, addUserToChatAction, addUserToChatCb }: UserAddDialogProps) {
