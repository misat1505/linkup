...
export function StatusFilterDropdown({ me, table }: StatusFilterDropdownProps) {
	...
	const getButtonText = (): React.ReactNode => {
		...
		if (filterValue === "Accepted")
			return <TRANSLATION_COMPONENT translationKey="friends.filter.statuses.input.accepted" />;
		if (filterValue === "Awaiting me")
			return <TRANSLATION_COMPONENT translationKey="friends.filter.statuses.input.awaiting-me" />;
		if (filterValue === "Awaiting other")
			return (
				<TRANSLATION_COMPONENT translationKey="friends.filter.statuses.input.awaiting-other" />
			);
		return <TRANSLATION_COMPONENT translationKey="friends.filter.statuses.input.all" />;
	};
			...
			<DropdownMenuContent align="end">
				<DropdownMenuItem onClick={() => table.getColumn("status")?.setFilterValue("")}>
					<TRANSLATION_COMPONENT
						...
						values={{
							count: String(Object.values(counts).reduce((acc, curr) => acc + curr, 0)),
						}}
					/>
				</DropdownMenuItem>
				<DropdownMenuItem onClick={() => table.getColumn("status")?.setFilterValue("Accepted")}>
					<TRANSLATION_COMPONENT
						translationKey="friends.filter.statuses.accepted"
						values={{ count: String(counts.accepted) }}
					/>
				</DropdownMenuItem>
				<DropdownMenuItem onClick={() => table.getColumn("status")?.setFilterValue("Awaiting me")}>
					<TRANSLATION_COMPONENT
				...
				<DropdownMenuItem
					onClick={() => table.getColumn("status")?.setFilterValue("Awaiting other")}
				>
					<TRANSLATION_COMPONENT
