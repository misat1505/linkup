...
export type StatusCellProps = {
	...
	me: User;
	acceptFriendshipAction: (id1: User["id"], id2: User["id"]) => Promise<Friendship>;
...
function StatusDropdown({
	...
	const handleAcceptFriendship = async () => {
		const fr = await acceptFriendshipAction(friendship.requester.id, friendship.acceptor.id);
		acceptFriendshipCb?.(fr);

		const otherUser = isMineRequest ? friendship.acceptor : friendship.requester;
	...
	const handleDeleteFriendship = async () => {
		await deleteFriendshipAction(friendship.requester.id, friendship.acceptor.id);
	...
	dropdownItems.push(
		<DropdownMenuItem onClick={handleDeleteFriendship} className="!text-red-500">
			<FaTrash />
			<span>
	...
	return (
		<DropdownMenu>
			<Tooltip
				content={<TRANSLATION_COMPONENT translationKey="friends.cells.actions.trigger.tooltip" />}
			>
				<span>
