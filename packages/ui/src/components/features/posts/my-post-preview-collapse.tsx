...
export function MyPostPreviewCollapse({ children, useTheme }: MyPostPreviewCollapseProps) {
	const { theme } = useTheme();
	const [isExpanded, setIsExpanded] = useState(false);

	return (
		<div
			className={cn("relative m-auto my-4 w-[95%] overflow-hidden px-4 pb-4 lg:w-[60%]", {
				"max-h-72": !isExpanded,
			})}
