...
export function PostCommentSectionLayout({
	...
	const getTooltipText = (message: Message) => {
		...
		return t("posts.comments.tooltip.messageInfo", {
			name,
			date: message.createdAt.toLocaleDateString(t("posts.comments.tooltip.locale")),
			time: message.createdAt.toLocaleTimeString(t("posts.comments.tooltip.locale")),
		});
	};
...
export function ResponseSetButton({
	...
	return (
		<Tooltip
			content={<TRANSLATION_COMPONENT translationKey="posts.comments.buttons.reply.tooltip" />}
		>
	...
	return (
		<div className="flex gap-x-1 self-stretch">
			{new Array(level).fill(0).map((_, idx) => (
				<div key={idx} className="w-0.5 self-stretch bg-slate-400 dark:bg-slate-600" />
			))}
		</div>
	);
}
