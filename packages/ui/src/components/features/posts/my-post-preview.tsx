...
import { MyPostPreviewCollapse, MyPostPreviewCollapseProps } from "./my-post-preview-collapse";
import MyPostPreviewNotPrerenderedContentWrapper from "./my-post-preview-not-prerendered-content-wrapper";

type IsPrerendered = { isPrerendered: true; renderedContent: string } | { isPrerendered: false };
	...
	return (
		<MyPostPreviewCollapse useTheme={useTheme}>
			...
			{rest.isPrerendered ? (
				...
				<MyPostPreviewNotPrerenderedContentWrapper content={post.content} useTheme={useTheme} />
			)}
			<PostActions
				postId={post.id}
