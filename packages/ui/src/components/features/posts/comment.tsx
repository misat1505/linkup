...
import { timeDifference, useGetReadableCommentUploadDate } from "../../../utils/time-difference";
...
export function Comment({ message }: { message: Message }) {
	...
	const uploadDate = useGetReadableCommentUploadDate(timeDifference(message.createdAt), t);
		...
		<div className="flex gap-x-2 p-2">
			...
			<div className="max-w-[calc(100%-4rem)]">
				<h2 className="text-nowrap">
					<span className="font-semibold">{createFullName(message.author)} </span>
...
function MultimediaDisplay({ files, chatId }: { files: File[]; chatId: Chat["id"] }) {
...
function MultimediaDisplayItem({ file, chatId }: { file: File; chatId: Chat["id"] }) {
	...
	if (type === "video")
		return <ProtectedVideo src={buildFileURL(file.url, { type: "chat-message", id: chatId })} />;

	return <ProtectedFile src={buildFileURL(file.url, { type: "chat-message", id: chatId })} />;
}
