...
export class FriendService {
	static async getMyFriendships(): Promise<Friendship[]> {
		...
		return res.friendships.filter((fr: Friendship) => fr.requester.id !== fr.acceptor.id);
	}
	...
	static async deleteFriendship(requesterId: User["id"], acceptorId: User["id"]): Promise<void> {
