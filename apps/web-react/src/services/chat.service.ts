...
import { ChatFormType, NewGroupChatFormType } from "@/validators/chat.validators";
...
export class ChatService {
	...
	static async addUserToChat(chatId: Chat["id"], userId: User["id"]): Promise<UserInChat> {
	...
	static async createPrivateChat(user1: User["id"], user2: User["id"]): Promise<Chat> {
		try {
	...
	static async createMessage(chatId: Chat["id"], payload: ChatFormType): Promise<Message> {
