...
import { decodeHTMLEntities, sanitizeMarkdownWithCodeBlocks } from "@/utils/editor-utils";
...
const EditorContext = createContext<EditorContextProvidedValues | undefined>(undefined);

// eslint-disable-next-line react-refresh/only-export-components
export const useEditorContext = () => {
	const context = useContext(EditorContext);
	if (context === undefined) throw new Error("useEditorContext called outside its provider.");
	return context;
};

const EditorProvider = ({ children, ...props }: EditorContextProps) => {
	const queryClient = useQueryClient();
	const [markdown, setMarkdown] = useState(props.variant === "update" ? props.post.content : "");
