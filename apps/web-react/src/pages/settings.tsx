...
export default function Settings() {
	...
	const { data, isLoading } = useQuery({
		...
		queryFn: () =>
			FileService.downloadFile(buildFileURL(me!.photoURL, { type: "avatar" }), me!.photoURL),
	});
	...
	const onSubmit: SubmitHandler<SignupFormType> = async (data: SignupFormEntries) => {
		try {
	...
	return (
		<div className="my-auto min-h-[calc(100vh-5rem)] w-full grid-cols-2 px-12 xl:grid mb-4 xl:mb-0">
			...
			<div className="col-span-1 mx-auto my-auto h-fit w-fit rounded-lg bg-transparent p-4 shadow-2xl shadow-black">
				<SignupFormProvider type="modify" onSubmit={onSubmit} defaultValues={defaultValues}>
					<SignupForm />
				</SignupFormProvider>
			</div>
		</div>
