...
function ChatNameAndImage() {
	...
	const file = useMemo(() => (fileData ? URL.createObjectURL(fileData) : null), [fileData]);

	return (
		<div className="flex flex-col items-center justify-between">
			<Input
				placeholder={t("chats.create-new-chat.group.form.inputs.name.placeholder")}
				className="my-2"
				{...register("name")}
			/>
			<div>
...
function UserSearch() {
	...
	const handleClick = (user: User) => (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
		e.preventDefault();
		appendUser(user);
	};

	const filteredUsers = data?.filter((user) => !users.some((u) => u.id === user.id));

	return (
		<div>
			<Input
				placeholder={t("chats.create-new-chat.group.form.inputs.search.placeholder")}
				className="my-2"
...
function SelectedUsers() {
	...
	return (
		<div>
			<h2 className="my-4 text-center font-semibold">{t("chats.create-new-chat.invited-users")}</h2>
