...
describe("chats spec", () => {
	...
	const sendMessage = (text: string, options = defaultOptions) => {
		...
		const messageText = openChatButton.children().eq(1).children().eq(1).children().eq(1);
	...
	it("sends message to chat", () => {
		...
		const openChatButton = cy.get("[data-testid=cy-chat-nav]").children().eq(1).children().first();
	...
	it("sends message to chat - searchbox navigation", () => {
		...
		const results = cy.get("[data-testid=cy-nav-search-results]").children().eq(1);
