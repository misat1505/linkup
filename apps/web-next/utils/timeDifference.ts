...
export function getStatus(
  ...
  t: (dotString: TranslationPath) => string,
): StatusType {
...
export function useGetReadableCommentUploadDate(diff: TimeDifference): string {
  ...
  if (days >= 365)
    return t("common.time.years", { count: String(Math.floor(days / 365)) });
  if (days >= 30)
    return t("common.time.months", { count: String(Math.floor(days / 30)) });
  if (days >= 7)
    return t("common.time.weeks", { count: String(Math.floor(days / 7)) });
  if (days > 0) return t("common.time.days", { count: String(days) });
  if (hours > 0) return t("common.time.hours", { count: String(hours) });
  if (minutes > 0) return t("common.time.minutes", { count: String(minutes) });
