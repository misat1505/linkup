	...
	return (
		<button type="submit" disabled={isDisabled} data-testid="cy-chat-footer-button">
			{isSubmitting ? (
				...
				<Tooltip content={<I18nText translationKey="posts.comments.form.submit.tooltip" />}>
					<span>
						<IoSend
...
function ResponseDisplayer() {
	...
	const getReplyText = (): React.ReactNode => {
		if (response!.author.id === me!.id)
			return <I18nText translationKey="posts.comments.form.reply.to.me" />;
		return (
			<I18nText
				translationKey="posts.comments.form.reply.to.other"
				values={{
	...
	return (
		<div className="flex w-full items-center justify-between">
			...
			<Tooltip content={<I18nText translationKey="posts.comments.form.reply.remove.tooltip" />}>
	...
	return (
		<>
			...
			<button onClick={clickInput}>
				<Tooltip content={<I18nText translationKey="posts.comments.form.inputs.file.tooltip" />}>
					<span>
			...
			{files.map((file, id) => (
				<button key={id} className="group relative h-40 w-40" onClick={handleRemoveFile(id)}>
					...
					<div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 text-white opacity-0 transition-opacity duration-300 group-hover:cursor-pointer group-hover:opacity-100">
						<I18nText translationKey="posts.comments.form.inputs.file.remove" />
					</div>
				</button>
			))}
		</div>
...
function FileDisplayerItem({ file }: { file: File }) {
	...
	if (type === "image")
		return <Image src={URL.createObjectURL(file)} fill className="object-cover" alt="image" />;
