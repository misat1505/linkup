...
async function getSignedUrlCachedUnsafe(filename: string, query: string = "") {
	...
	if (!result.ok) {
		console.error("Failed to fetch signed URL:", result.status, result.statusText);
		return "";
	}
...
export async function replaceLinksCachedUnsafe(markdown: string): Promise<string> {
	const baseApiURL = FILE_API.defaults.baseURL!;

	// eslint-disable-next-line no-useless-escape
	const escapedBaseUrl = baseApiURL.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&");

	const fileLinkRegex = new RegExp(`${escapedBaseUrl}/([a-zA-Z0-9._-]+)(\\?[^\\s)"]*)?`, "g");
