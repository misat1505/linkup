...
export default function ChatContent() {
	...
	const { messages, isLoading, incomeMessage, fetchNextPage, isFetchingNextPage, hasNextPage } =
		useChatContext();
	const { bottomRef, containerRef, handleScroll, wasAtBottomRef, scrollToBottom } = useChatScroll();
	...
	useEffect(() => {
		...
		if (inView && hasNextPage && !isFetchingNextPage && now - lastFetchRef.current > 1000) {
			lastFetchRef.current = now;
			fetchNextPage();
		}
	}, [inView, hasNextPage, isFetchingNextPage, fetchNextPage]);

	const styles = "flex-grow overflow-hidden bg-slate-100 dark:bg-slate-900 pt-2 relative";
	...
	return (
		<div className={styles}>
			...
			{incomeMessage && !wasAtBottomRef.current && (
				<IncomeMessage key={incomeMessage.id} message={incomeMessage} onclick={scrollToBottom} />
			)}
		</div>
	);
}
