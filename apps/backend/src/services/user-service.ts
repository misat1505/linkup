...
export class UserService {
	 ...
	 */
	async getUserByLogin(login: UserWithCredentials["login"]): Promise<UserWithCredentials | null> {
		const user: UserWithCredentials | null = await this.prisma.user.findFirst({
			where: { login },
		});
	 ...
	 * @returns The user object, or `null` if the user is not found.
	 */
	async getUser(id: UserWithCredentials["id"]): Promise<UserWithCredentials | null> {
		const user: UserWithCredentials | null = await this.prisma.user.findFirst({
			where: { id },
		});
