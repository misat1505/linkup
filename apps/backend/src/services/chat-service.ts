...
import { messageWithoutResponseSelect } from "@/utils/prisma/message-without-response-select";
...
export class ChatService {
	...
	async getChatMessages(
		...
		if (lastMessage && lastMessage.chatId !== chatId) throw new Error("Message not in chat.");
	 ...
	 */
	async getPrivateChatByUserIds(id1: string, id2: string): Promise<Chat | null> {
		const result = await this.prisma.chat.findMany({
			where: {
				type: "PRIVATE",
				users: {
