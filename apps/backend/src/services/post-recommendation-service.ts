...
export class PostRecommendationService {
	...
	async fetchFriendsPostsOnly(
		...
		const createdAtFilter = lastPost?.createdAt ? { lt: lastPost.createdAt } : undefined;
	...
	async getRecommendedPosts(
		...
		if (lastPost && !friends.includes(lastPost.author.id)) {
			return await this.fetchNonFriendsPostsOnly(lastPost.createdAt, friends, userId, limit);
		}

		const friendsPosts = await this.fetchFriendsPostsOnly(lastPost, friends, limit);

		const remainingPosts = limit - friendsPosts.length;

		if (remainingPosts === 0) return friendsPosts.map((p) => Post.parse(p));

		const othersPostsCreatedAtFilter = PostRecommendationService.getOthersPostsCreatedAtFilter(
			lastPost,
			friendsPosts,
			friends,
		);
