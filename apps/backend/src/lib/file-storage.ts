...
export class FileStorage {
	...
	constructor(config?: {
		...
		const isS3Local = env.S3_ENDPOINT.includes("localhost") || env.S3_ENDPOINT.includes("minio");
	 ...
	 */
	async uploadFile(fileBuffer: Buffer, contentType: string, path: string): Promise<string> {
