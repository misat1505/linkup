...
describe("getChatMessages", () => {
	...
	it("retrieves chat messages without responseId", async () => {
		...
		expect(respond).toHaveBeenCalledWith(StatusCodes.OK, expect.anything());
		expect(mockChatService.getChatMessages).toHaveBeenCalledWith("someId", "message1", 5);
	});
	...
	it("retrieves chat messages for specific responseId", async () => {
		...
		expect(respond).toHaveBeenCalledWith(StatusCodes.OK, expect.anything());
		expect(mockChatService.getPostChatMessages).toHaveBeenCalledWith("someId", "response123");
	});
	...
	it("handles null responseId and returns null", async () => {
		...
		expect(respond).toHaveBeenCalledWith(StatusCodes.OK, expect.anything());
		expect(mockChatService.getPostChatMessages).toHaveBeenCalledWith("someId", null);
	});
