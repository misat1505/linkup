...
describe("ChatService", () => {
	...
	describe("createPrivateChat", () => {
		it("creates new private chat", async () => {
			await transactionProvider(async ({ tx, seed }) => {
				const chatService = new ChatService(tx);
				const result = await chatService.createPrivateChat(seed.users[1].id, seed.users[0].id);
	...
	describe("createGroupChat", () => {
		it("creates new group chat", async () => {
			await transactionProvider(async ({ tx, seed }) => {
				const chatService = new ChatService(tx);
				const result = await chatService.createGroupChat([seed.users[1].id], "name", "photo.webp");
	...
	describe("getPostChatMessages", () => {
		it("retrieves post-related chat messages by ID", async () => {
			await transactionProvider(async ({ tx, seed }) => {
				...
				const result = await chatService.getPostChatMessages(seed.chats[0].id, null);
				expect(result.length).toBe(1);
				result.forEach((message) => {
					Message.strict().parse(message);
				});
