...
describe("PostRecommendationService", () => {
	function _initService(tx: PrismaClientOrTransaction): PostRecommendationService {
	...
	describe("getLastPost", () => {
		...
		it("throws error for non-existent post", async () => {
			await transactionProvider(async ({ tx }) => {
				...
				await expect(postRecommendationService.getLastPost("post1")).rejects.toThrow(
					"Last post not found.",
				);
			});
		});
	...
	describe("fetchNonFriendsPostsOnly", () => {
		it("fetches non-friends' posts with deadline", async () => {
			await transactionProvider(async ({ tx }) => {
				...
				expect(result).toEqual([mockPost]);
				expect(fetchSpy).toHaveBeenCalledWith(deadline, [FRIEND1.id], USER.id, 10);
				fetchSpy.mockRestore();
			});
		});
	});
	...
	describe("getRecommendedPosts", () => {
		it("returns only non-friends' posts for non-friend lastPost", async () => {
			await transactionProvider(async ({ tx }) => {
				...
				const fetchNonFriendsSpy = vi
					.spyOn(postRecommendationService, "fetchNonFriendsPostsOnly")
					.mockResolvedValue(mockPosts);

				const result = await postRecommendationService.getRecommendedPosts(USER.id, "post1", 10);
		...
		it("returns only friends' posts when limit reached", async () => {
			await transactionProvider(async ({ tx }) => {
				...
				const result = await postRecommendationService.getRecommendedPosts(USER.id, null, 1);
		...
		it("returns mixed friends and non-friends' posts", async () => {
			await transactionProvider(async ({ tx }) => {
				...
				const result = await postRecommendationService.getRecommendedPosts(USER.id, null, 2);
