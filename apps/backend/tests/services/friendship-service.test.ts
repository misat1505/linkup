...
describe("FriendshipService", () => {
	describe("getUserFriendships", () => {
		it("returns empty list for no friendships", async () => {
			await transactionProvider(async ({ tx, seed }) => {
				...
				const result = await friendshipService.getUserFriendships(seed.users[0].id);
		...
		it("retrieves user friendships", async () => {
			await transactionProvider(async ({ tx, seed }) => {
				const friendshipService = new FriendshipService(tx);
				await friendshipService.createFriendship(seed.users[0].id, seed.users[1].id);
	...
	describe("createFriendship", () => {
		it("creates new friendship", async () => {
			await transactionProvider(async ({ tx, seed }) => {
				const friendshipService = new FriendshipService(tx);
				const result = await friendshipService.createFriendship(seed.users[0].id, seed.users[1].id);
	...
	describe("acceptFriendship", () => {
		it("accepts pending friendship request", async () => {
			await transactionProvider(async ({ tx, seed }) => {
				const friendshipService = new FriendshipService(tx);
				await friendshipService.createFriendship(seed.users[0].id, seed.users[1].id);
				const result = await friendshipService.acceptFriendship(seed.users[0].id, seed.users[1].id);
	...
	describe("deleteFriendship", () => {
		it("deletes existing friendship", async () => {
			await transactionProvider(async ({ tx, seed }) => {
				const friendshipService = new FriendshipService(tx);
				await friendshipService.createFriendship(seed.users[0].id, seed.users[1].id);
				const result = await friendshipService.deleteFriendship(seed.users[0].id, seed.users[1].id);

				expect(result).toBeTruthy();

				const friendships = await friendshipService.getUserFriendships(seed.users[0].id);
